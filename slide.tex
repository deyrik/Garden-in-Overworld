...
\begin{frame}{Roteiro (1/2)}
  \footnotesize
  \begin{columns}
    \column{0.48\textwidth}
    \textbf{1. Fundamentos da Comunica\c{c}\~ao} \\
    \quad O que \'e? $\cdot$ Funcionalidades $\cdot$ Contexto \\
    \quad Protocolo $\cdot$ Sess\~ao exemplo \\
    \quad Modelo F\'isico $\cdot$ Modelo de Intera\c{c}\~ao
    \vfill
    \textbf{2. Serializa\c{c}\~ao e Mensagens} \\
    \quad Estrat\'egias $\cdot$ Serializa\c{c}\~ao \\
    \quad Framing $\cdot$ Pipeline \\
    \quad Comandos Cliente $\cdot$ Eventos Servidor
    \vfill
    \textbf{3. Arquitetura do Servidor} \\
    \quad Vis\~ao Geral $\cdot$ Arquitetura Geral \\
    \quad SRP $\cdot$ Diagrama \\
    \quad Socket TCP $\cdot$ Thread-per-Connection \\
    \quad Despachante $\cdot$ Timers $\cdot$ Slots \\
    \quad Arq. Multithreaded $\cdot$ Falhas \\
    \quad Race Condition
    \column{0.48\textwidth}
    \textbf{4. Funcionalidades do Jogo} \\
    \quad Mundo $\cdot$ Estados \\
    \quad RF01--RF08 (Inicializa\c{c}\~ao, Plantio, \\
    \quad \hfill Valida\c{c}\~ao, Colheita, Broadcast) \\
    \quad Func. Adicionais $\cdot$ Rastreabilidade \\
    \quad Casos de Uso UC01--UC10 \\
    \quad (Identificar, Mapa, Plantar, Colher, \\
    \quad \hfill Preparar, Semente, Chat, Cursor)
    \vfill
    \textbf{5. Cliente e Interface Gr\'afica} \\
    \quad Vis\~ao Geral $\cdot$ TCP \\
    \quad Proxy Manual $\cdot$ RMI \\
    \quad Terminal $\cdot$ C\'opia Local \\
    \quad GUI PyQt6 $\cdot$ MVC \\
    \quad Widgets $\cdot$ Di\'alogos \\
    \quad OuvinteThread $\cdot$ Eventos $\cdot$ Controller
  \end{columns}
\end{frame}

\begin{frame}{Roteiro (2/2)}
  \footnotesize
  \begin{columns}
    \column{0.48\textwidth}
    \textbf{6. Descoberta de Servidor} \\
    \quad Motiva\c{c}\~ao $\cdot$ UDP vs TCP \\
    \quad AnunciadorUDP $\cdot$ DialogConexao
    \vfill
    \textbf{7. Temporadas} \\
    \quad Ciclo $\cdot$ Demanda \\
    \quad Implementa\c{c}\~ao $\cdot$ Geada $\cdot$ Transi\c{c}\~ao
    \vfill
    \textbf{8. Conceitos Avan\c{c}ados de SD} \\
    \quad Mapa de Conceitos $\cdot$ M\'etricas \\
    \quad Idempot\^encia $\cdot$ Transpar\^encia \\
    \quad Exce\c{c}\~oes $\cdot$ Consist\^encia \\
    \quad RMI vs Sockets $\cdot$ Middleware
    \vfill
    \textbf{9. Limita\c{c}\~oes e Trab. Futuros} \\
    \quad Race Condition $\cdot$ Falhas \\
    \quad Persist\^encia $\cdot$ Protocolo \\
    \quad Sincroniza\c{c}\~ao $\cdot$ Robustez \\
    \quad Seguran\c{c}a $\cdot$ Arquitetura
    \column{0.48\textwidth}
    \textbf{10. Execu\c{c}\~ao} \\
    \quad Pr\'e-requisitos $\cdot$ Docker \\
    \quad Servidor $\cdot$ Cliente \\
    \quad Multi-m\'aquina $\cdot$ Makefile
    \vfill
    \textbf{11. Evolu\c{c}\~ao e Conclus\~ao} \\
    \quad Branches $\cdot$ Front-End $\cdot$ Middleware \\
    \quad Commits $\cdot$ Reposit\'orio \\
    \quad O que foi constru\'ido $\cdot$ Desafios \\
    \quad SD Aplicados $\cdot$ Li\c{c}\~oes $\cdot$ C\'odigo
  \end{columns}
\end{frame}
...
\begin{frame}{Demonstra\c{c}\~ao -- Inicializa\c{c}\~ao e Conex\~ao}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{Inicializa\c{c}\~ao do Servidor (\texttt{make NS})}
      \centering
      \includegraphics[width=0.75\linewidth]{Artefatos/server_init.png}
    \end{block}
    \footnotesize Servidor gera o mapa ASCII no console e inicia o anúncio de sua presença na rede local via broadcast UDP na porta 37020.
    
    \column{0.48\textwidth}
    \begin{block}{Descoberta pelo Cliente (\texttt{make NC})}
      \centering
      \includegraphics[width=0.75\linewidth]{Artefatos/client_conn.png}
    \end{block}
    \footnotesize Cliente GUI escuta os anúncios UDP locais, lista os servidores disponíveis e solicita o nickname para a conexão.
  \end{columns}
\end{frame}

\begin{frame}{Demonstra\c{c}\~ao -- Logs de Conex\~ao e A\c{c}\~ao no Servidor}
  \begin{columns}[T]
    \column{0.58\textwidth}
    \centering
    \includegraphics[width=0.95\linewidth]{Artefatos/server_conn.png}
    
    \column{0.38\textwidth}
    \begin{block}{Logs do Servidor}
      \footnotesize
      Acompanhamento das ações em tempo real:
      \begin{itemize}
        \item Identificação do endereço IP e porta do cliente.
        \item Registro de novo agricultor conectando-se ao slot do jogo.
        \item Recebimento e processamento dos comandos em formato JSON (ex: \texttt{CURSOR}, \texttt{PREPARAR}).
      \end{itemize}
    \end{block}
  \end{columns}
\end{frame}

\begin{frame}{Demonstra\c{c}\~ao -- Interface Principal da Gameplay}
  \centering
  \includegraphics[width=0.8\linewidth]{Artefatos/gameplay_start.png}
  
  \vspace{0.1cm}
  \footnotesize Visão geral do cliente no início da Temporada 1 (Primavera), mostrando o mapa quadriculado da fazenda, estoque global, painel de ações/sementes e logs detalhados de rede no painel esquerdo.
\end{frame}

\begin{frame}{Demonstra\c{c}\~ao -- Cultivo e Din\^amica de Temporadas}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{Ciclo de Cultivo Ativo}
      \centering
      \includegraphics[width=0.8\linewidth]{Artefatos/gameplay_active.png}
    \end{block}
    \footnotesize Visualização das plantações crescendo no mapa (trigo, arroz, cana) e o status do estoque global.
    
    \column{0.48\textwidth}
    \begin{block}{Novas Regras da Temporada (Ver\~ao)}
      \centering
      \includegraphics[width=0.8\linewidth]{Artefatos/gameplay_tutorial.png}
    \end{block}
    \footnotesize Notificação tutorial informando novas regras ao mudar de estação, como tempo reduzido e inclusão de milho.
  \end{columns}
\end{frame}

\begin{frame}{Demonstra\c{c}\~ao -- Protocolo e Fluxo de Mensagens}
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{block}{Comandos de Gameplay}
      \centering
      \includegraphics[width=0.8\linewidth]{Artefatos/gameplay_commands.png}
    \end{block}
    \footnotesize Mensagens trocadas no final do ciclo, registrando comandos como \texttt{COLHER} e o status \texttt{VITORIA} ou \texttt{DERROTA}.
    
    \column{0.48\textwidth}
    \begin{block}{Sincroniza\c{c}\~ao via Sockets}
      \centering
      \includegraphics[width=0.8\linewidth]{Artefatos/gameplay_tick.png}
    \end{block}
    \footnotesize Loop constante de mensagens \texttt{TICK\_TIMER} enviadas pelo servidor a cada segundo para atualizar o tempo restante do jogo.
  \end{columns}
\end{frame}
